...
test('challenge worker creation, capability negotiation and owner-scoped recovery hide all future data',async()=>{
 ...
 try{
  const origin=(await mf.ready).origin,control=createServiceControl({url:origin});assert.ok((await control.capabilities()).features.includes('takeover-challenge-v1'));
