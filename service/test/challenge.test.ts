...
test('v6 checkpoint preserves all four hands, winners, score history, and training comparison',()=>{
 const before=JSON.stringify(record),saved=start(),core=new TableCore(saved);core.join(2,now);
 ...
 assert.equal(core.windows[2]!.deadlineAtMs-now,120000);assert.equal(core.state.wallOrder.length-core.state.wallIndex,23);
...
test('all nine initial discards finish legally with identical results after checkpoint restore',()=>{
 for(const first of ['1m','2m','3m','4m','6m','7m','8m','3p','5p']){
  const play=(restore:boolean)=>{let core=new TableCore(start());core.join(2,now);let n=0;
...
test('live views and AI decisions contain no future draws, reference result, seed, or other concealed hands',()=>{
 const core=new TableCore(start());core.join(2,now);const v=core.view(2).entries,summary=v.find(([k])=>k==='match')![2].challenge;
...
test('unavailable original draw skips only remaining-wall entries and resumes the later own draw',()=>{
 const core=new TableCore(start());core.join(2,now);const s=core.state;
...
test('timeout follows source policy, duplicate submissions do not advance twice, stale actions cannot mutate',()=>{
 const core=new TableCore(start());core.join(2,now);const id=core.catalog(2).publicActions.find((a:any)=>a.tile==='6m').legalActionId;
...
test('evaluation is version-bound and normal practice does not inherit a private challenge guide',async()=>{
 ...
 const saved=start(),core=new TableCore(saved);core.join(2,now);
