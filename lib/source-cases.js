...
export function createSourceCaseCatalog(directory=path.resolve(import.meta.dirname,'../.local/cases')) {
  ...
  return {
    ...
    list:()=>[...cases.values()].map(c=>({caseId:c.caseId,title:c.title,count:c.data.snapshots.length,players:c.data.players,keyIndex:c.keyIndex,keySeat:c.keySeat,hasLessons:(c.lessons?.length??0)>0,notice:c.notice})),
