...
function routeMethod(pathname) {
  ...
  if (["coach/start","coach/status","history/open","practice/start","replays/export","replays/import","replays/share","replays/shares","replays/revokeShare","replays/revokeSeat","replays/users"].some(path=>pathname===`${API_PREFIX}/${path}`)) return "POST";
...
export function createDshMahjongHttpHandler(controllerPromise, options = {}) {
  ...
  return async function dshMahjongHttpHandler(req, res) {
    ...
    try {
      ...
      if(url.pathname===`${API_PREFIX}/sources`){oneQuery(url,"unused");writeJson(res,200,controller.sourceCases());return;}
