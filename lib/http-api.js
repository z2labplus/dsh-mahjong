...
function routeMethod(pathname) {
  ...
  if (["challenge/start","challenge/begin","coach/start","coach/status","history/open","practice/start","replays/export","replays/import","replays/share","replays/shares","replays/revokeShare","replays/revokeSeat","replays/users"].some(path=>pathname===`${API_PREFIX}/${path}`)) return "POST";
...
export function createDshMahjongHttpHandler(controllerPromise, options = {}) {
  ...
  return async function dshMahjongHttpHandler(req, res) {
    ...
    try {
      ...
      if(url.pathname===`${API_PREFIX}/challenge/begin`){exactKeys(body,["sessionId"]);writeJson(res,200,{ok:true,state:await controller.beginChallenge(body)});return;}
