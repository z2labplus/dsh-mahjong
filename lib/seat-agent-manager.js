...
class SeatAgentManager {
  ...
  async #activate(seat, decision) {
    try {
      ...
      if(seat.config?.provider==="script" && seat.config?.model==="source-priority-v1"){
        const envelope=JSON.parse(decision.stateBlock);
        if(envelope.sourcePriority?.policy!=="source-priority-v1")throw fail("CHALLENGE_POLICY_MISSING","challenge decision has no source-priority action");
        seat.handle.agent.session.append("user/message",message,{surfaceOp:"append"});
        await this.#executeSubmit(seat,{decisionId:decision.decisionId,actionId:envelope.sourcePriority.legalActionId},{signal:undefined,concludeTurn(){}});
        this.#resolve(decision,"prompted");return;
      }
