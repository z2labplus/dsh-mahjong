...
export function createServiceControl(options = {}) {
  ...
  return Object.freeze({
    ...
    capabilities: async () => {const response=await fetchImpl(`${origin}/health`,{redirect:"error",signal:AbortSignal.timeout(5000)});return response.ok?response.json():{};},
    startChallenge: ({record,keySeq,seat,seed,ownerApiToken}) => request(requestIdFactory(),"PUT",ownerApiToken,{record,keySeq,seat,seed},undefined,"/challenge"),
    beginChallenge: ({gameId,ownerApiToken}) => request(gameId,"POST",ownerApiToken,{action:"begin"},undefined,"/challenge"),
    ...
    createTable: ({ ownerApiToken, tableName, timeoutSeconds, seats, ruleset = "blood", ruleOptions = {}, signal }) => request(requestIdFactory(), "PUT", ownerApiToken, {
      ...
      seats: seats.map(seat => seat.kind === "human"
        ? { seat: seat.seat, kind: "human", initialPoints: seat.initialPoints, ...(seat.owner ? { owner: true } : {}) }
        : { seat: seat.seat, kind: "ai", initialPoints: seat.initialPoints, modelId: seat.model, modelLabel: seat.modelLabel }),
    }, signal),
