...
function publicRecord(request, created, now) {
  ...
  return {
    ...
    game: {
      ...
      ...(created.challenge ? {challenge:created.challenge} : {}),
...
export class DshMahjongGameController {
  ...
  state(sessionId) {
    ...
    const game = record.game === undefined
      ...
      : Object.freeze({
          ...record.game,
          ...(record.game.mode==="challenge"?{challenge:live?.snapshot?.entries?.find(([kind])=>kind==="match")?.[2]?.challenge??record.game.challenge}:{}),
          ...(record.game.mode==="challenge"?{challengeCaseId:record.game.challengeCaseId??this.#sourceCases.list().find(c=>c.keySeat===record.game.challenge?.seat&&this.#sourceCases.get(c.caseId).data.provenance?.sourceRecordSha256===record.game.challenge?.sourceHash)?.caseId}:{}),
  ...
  async startChallenge({sessionId,caseId}){
    this.#agentForSession(sessionId);
    if(this.#store.get(sessionId))throw fail("GAME_LOCKED","请在新的挑战会话中开始");
    return this.#exclusive(sessionId,async()=>{
      this.#sourceCases.reload(caseId);
      const c=this.#sourceCases.get(caseId);
      if(!c.record||!Number.isInteger(c.keyIndex)||!Number.isInteger(c.keySeat))throw fail("CHALLENGE_SOURCE_UNAVAILABLE","该案例尚未准备好完整接手数据");
      const created=await this.#control.startChallenge({record:c.record,keySeq:c.data.snapshots[c.keyIndex].seq,seat:c.keySeat,seed:20260910,ownerApiToken:this.#ownerApiToken});
      const request={sessionId,tableName:created.tableName,ruleset:created.ruleset,ruleOptions:created.ruleOptions,seats:created.seats.map(seat=>({...seat,provider:seat.kind==="ai"?"script":undefined,model:seat.modelId}))};
      const record=publicRecord(request,created,this.#now),live=this.#newVolatile(record);
      record.game.challengeCaseId=caseId;
      this.#volatile.set(sessionId,live);await this.#store.put(sessionId,record);
      try{await this.#activate(record,created,live);}catch(error){await this.#disposeLive(live);await this.#markError(record,safeErrorCode(error,"CHALLENGE_START_FAILED"),true);throw error;}
      const agent=this.#agentForSession(sessionId);this.#installAgent(agent);
      this.#openStaticConversation(agent,"挑战已准备好，点击上方「开始挑战」后再出牌。你接替原选手，轮到你时不限时，不会自动替你出牌。其他座位优先沿用原谱。接手后的计分采用训练规则；结束后查看净得分对照和出牌效率。问答不会代你出牌。");
  ...
  async beginChallenge({sessionId}){
    return this.#exclusive(sessionId,async()=>{
      const record=this.#store.get(sessionId);
      if(record?.phase!=="active"||record.game?.mode!=="challenge")throw fail("CHALLENGE_NOT_FOUND","本会话不是可开始的挑战");
      const result=await this.#control.beginChallenge({gameId:record.game.gameId,ownerApiToken:this.#ownerApiToken});
      record.game.challenge=result.challenge;await this.#store.put(sessionId,record);
      const state=this.state(sessionId);
      return {...state,game:{...state.game,challenge:result.challenge}};
    });
  }
  ...
  async sourceCases(){
    let available=false;try{available=(await this.#control.capabilities?.())?.features?.includes("takeover-ready-v1")===true;}catch{}
    return {ok:true,items:this.#sourceCases.list().map(c=>({...c,challengeServiceReady:available}))};
  }
