...
export class DshMahjongGameController {
  ...
  async startChallenge({sessionId,caseId}){
    ...
    return this.#exclusive(sessionId,async()=>{
      ...
      this.#openStaticConversation(agent,"已进入接手续打训练。你接替原选手自由操作，其他座位优先沿用原谱。接手后的计分采用训练规则；结束后查看净得分对照和出牌效率。问答不会代你出牌。");
  ...
  async sourceCases(){
    let available=false;try{available=(await this.#control.capabilities?.())?.features?.includes("takeover-challenge-v1")===true;}catch{}
