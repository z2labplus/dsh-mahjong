...
3. 一栏、默认两栏和三栏详情能否复用同一个 `/hand/` iframe 与牌局状态。
...
- “提问这一步”将同一个 16:9 牌桌缩为紧凑状态并聚焦原生 composer；“聚焦牌桌”恢复大状态。
- 牌桌本体无圆角、边框、阴影、模糊、遮罩或裁切，且不覆盖 composer、审批、`ask_user` 或停止生成控件。
- 一栏、两栏、三栏及牌桌大小切换不重建 iframe，不丢失牌局、问题和回答。
...
- 已通过：收起/展开左侧栏可在一栏与默认两栏间切换；牌桌重新居中，iframe 始终为 1 个且 URL 不变。
- 已通过：大/紧凑状态切换、会话切换均保留同一个 iframe 挂载边界；切换会话时只重绑几何 observer。
- 已通过：原生 composer 保留且只有 1 个，插件内没有 textarea；“提问这一步”会聚焦原生 composer；运行时无 error，验收期间仅记录到重启专用 Harness 服务所产生的宿主重连 warning。
- 待完成：由真实详情内容触发三栏后的最终浏览器截图；当前 Harness `0.1.0-rc.8` 已有 `ctx.layout.openDetails()` 与三栏网格，但当前工具行未暴露稳定的详情打开入口。
